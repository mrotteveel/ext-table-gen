...
Firebird external table files have a binary format, and support datatypes other than `CHAR` for columns.
...
The storage of the integral datatypes has alignment requirements.
...
In Java, the Firebird datatypes can be mapped as follows:
...
_ext-table-gen_ will support the four integer datatypes (`SMALLINT`, `INTEGER`, `BIGINT` and `INT128`).
...
In the XML config, the datatypes will be represented by elements:
...
The documentation must be updated to explain how to use these new datatypes, and to describe the configuration changes
