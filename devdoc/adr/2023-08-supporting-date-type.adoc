...
Firebird has a `DATE` datatype, which is a 32-bit integer specifying the number of days since 17 November 1858 (as a Modified Julian Date).
...
Java has various datatypes for representing datetime information.
...
_ext-table-gen_ will support the `DATE` datatype.
...
In the XML config, the datatype will be represented by element `date`.

In Java code, the datatype will be represented by the type `FbDate`.
...
The name includes _time_ because we expect it can be reused for other datetime datatypes.
