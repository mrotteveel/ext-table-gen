...
_ext-table-gen_ will support the `FLOAT` and `DOUBLE PRECISION` datatypes.
...
In the XML config, the datatypes will be represented by elements `float` and `doublePrecision`, without any attributes.
...
The documentation must be updated to explain how to use this new datatypes, and to describe the configuration changes.
