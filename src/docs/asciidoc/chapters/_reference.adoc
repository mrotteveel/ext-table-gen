...
A datatype is a placeholder for an element corresponding to a specific Firebird datatype.
...
The `bigint` element represents the Firebird datatype https://firebirdsql.org/docs/chunk/en/refdocs/fblangref50/fblangref50-datatypes.html#fblangref50-datatypes-bigint[`BIGINT`^].
...
The `char` element represents the Firebird datatype https://firebirdsql.org/docs/chunk/en/refdocs/fblangref50/fblangref50-datatypes-chartypes.html#fblangref50-datatypes-chartypes-char[`CHAR`^].

See also <<ref-xml-varchar>>.
...
[TIP#ref-xml-char-vs-varchar]
.Choosing between `char` and `varchar`
====
When in doubt, use `char`, not `varchar`.

Given how its stored, using `varchar` will make the external table file a truly binary format.
A `VARCHAR` takes 2-3 bytes more than a `CHAR` column (2 bytes for actual length, 0-1 bytes for alignment)footnote:[Remaining bytes not used for the actual value are filled with 0x00 (NUL) as external tables are a fixed width binary format].

The benefit of using `VARCHAR` is that the resulting value (when queried) preserves its original length;
unlike `CHAR`, it is not padded with spaces to the declared length.

So, if you want or need values to preserve their original length, use `varchar`.
We think that is the only reason to choose `varchar` over `char`.
====

[float#ref-xml-date]
====== `date`

The `date` element represents the Firebird datatype https://firebirdsql.org/docs/chunk/en/refdocs/fblangref50/fblangref50-datatypes-datetime.html#fblangref50-datatypes-date[`DATE`^].
...
[float#ref-xml-decfloat]
====== `decfloat`

The `decfloat` element represents the Firebird datatype https://firebirdsql.org/file/documentation/chunk/en/refdocs/fblangref50/fblangref50-datatypes-floattypes.html#fblangref50-datatypes-decfloat[`DECFLOAT`^].

The default conversion parses the string using https://firebirdsql.github.io/decimal-java/javadoc/org.firebirdsql.decimal/org/firebirdsql/decimal/Decimal64.html#valueOf(java.lang.String,org.firebirdsql.decimal.OverflowHandling)[`Decimal64.valueOf(String, OverflowHandling)`^] and https://firebirdsql.github.io/decimal-java/javadoc/org.firebirdsql.decimal/org/firebirdsql/decimal/Decimal128.html#valueOf(java.lang.String,org.firebirdsql.decimal.OverflowHandling)[`Decimal128.valueOf(String, OverflowHandling)`^] of https://github.com/FirebirdSQL/decimal-java[`org.firebirdsql:decimal-java`^].
Indirectly, these `valueOf` methods uses the constructor https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/math/BigDecimal.html#%3Cinit%3E(java.lang.String)[`BigDecimal(String)`^], with extra support for the {plus}/-Infinity, {plus}/-NaN, and {plus}/-sNaN values.
Contrary to the <<ref-xml-parsebigdecimal>> converter, this converter can also parse strings in scientific notation.

.Attributes
[horizontal]
`precision`::
Integer in `16` or `34` for the precision in decimal digits (required).
`onOverflow`::
Overflow handling (optional, defaults to `ROUND_TO_INFINITY`).
Possible values are:
+
--
[horizontal]
`ROUND_TO_INFINITY`:::
Overflow rounds to {plus}/-Infinity
`THROW_EXCEPTION`:::
Overflow throws a (subclass of) `ArithmeticException`
--
...
The `decimal` element represents the Firebird datatype https://firebirdsql.org/docs/chunk/en/refdocs/fblangref50/fblangref50-datatypes-fixedtypes.html#fblangref50-datatypes-decimal[`DECIMAL`^].
...
Precision greater than 18 requires Firebird 4.0 and higher.
`scale`::
Integer in the range [0, _precision_] for the scale in decimal digits (required). +
...
The `doublePrecision` element represents the Firebird datatype https://firebirdsql.org/docs/chunk/en/refdocs/fblangref50/fblangref50-datatypes-floattypes.html#fblangref50-datatypes-double[`DOUBLE PRECISION`^].
...
The `float` element represents the Firebird datatype https://firebirdsql.org/docs/chunk/en/refdocs/fblangref50/fblangref50-datatypes-floattypes.html#fblangref50-datatypes-float[`FLOAT`^].
...
The `int128` element represents the Firebird datatype https://firebirdsql.org/docs/chunk/en/refdocs/fblangref50/fblangref50-datatypes.html#fblangref50-datatypes-int128[`INT128`^].
...
The `integer` element represents the Firebird datatype https://firebirdsql.org/docs/chunk/en/refdocs/fblangref50/fblangref50-datatypes.html#fblangref50-datatypes-int[`INTEGER`^].
...
The `numeric` element represents the Firebird datatype https://firebirdsql.org/docs/chunk/en/refdocs/fblangref50/fblangref50-datatypes-fixedtypes.html#fblangref50-datatypes-numeric[`NUMERIC`^].
...
The `smallint` element represents the Firebird datatype https://firebirdsql.org/docs/chunk/en/refdocs/fblangref50/fblangref50-datatypes.html#fblangref50-datatypes-smallint[`SMALLINT`^].
...
The `time` element represents the Firebird datatype https://firebirdsql.org/docs/chunk/en/refdocs/fblangref50/fblangref50-datatypes-datetime.html#fblangref50-datatypes-time[`++TIME [WITHOUT TIME ZONE]++`^].
...
The `timestamp` element represents the Firebird datatype https://firebirdsql.org/docs/chunk/en/refdocs/fblangref50/fblangref50-datatypes-datetime.html#fblangref50-datatypes-timestamp[`++TIMESTAMP [WITHOUT TIME ZONE]++`^].
...
[float#ref-xml-varchar]
====== `varchar`

The `varchar` element represents the Firebird datatype https://firebirdsql.org/docs/chunk/en/refdocs/fblangref50/fblangref50-datatypes-chartypes.html#fblangref50-datatypes-chartypes-varchar[`VARCHAR`^].

See also <<ref-xml-char>> and <<ref-xml-char-vs-varchar>>.

.Attributes
[horizontal]
`length`::
Integer in the range [1, 32765] with length in Unicode codepoints (required).
+
For simplicity, the XSD reuses the definition of <<ref-xml-char>>.
Unfortunately, this means that the XSD actually enforces the range [1, 32767] (not [1, 32765]).
We may address this in the future.
+
The XSD also does not validate maximum length for multibyte character sets (e.g. for `encoding="UTF8"` the actual maximum length would be 8191, but this is not enforced).
`encoding`::
Encoding (character set) of the column, using Firebird character set names (required). +
The XSD uses an enum-type, `encoding`, with supported names.
...
The `converter` element -- if present -- defines a conversion from the CSV string value to a Firebird datatype value.
If absent, a default conversion will be applied, specific to the column datatype.
...
This converter step is only valid in <<ref-xml-decfloat>>, <<ref-xml-decimal>>, and <<ref-xml-numeric>>.
Contrary to the default converter of `decimal`, and `numeric`, this converter does not parse strings in scientific notation.
Contrary to the default converter of `decfloat`, this converter does not parse strings in scientific notation nor the special values ({plus}/-Infinity, {plus}/-NaN, {plus}/-sNaN).
...
The `ddl` element contains the Firebird https://firebirdsql.org/docs/chunk/en/refdocs/fblangref50/fblangref50-ddl-table.html#fblangref50-ddl-tbl-create[`CREATE TABLE`^] statement derived from <<ref-xml-externaltable>>.
