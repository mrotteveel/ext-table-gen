...
This option is ignored if <<ref-cmd-table-file,`--table-file`>> is not specified.
...
However, currently we handle this inconsistently, because in the other direction, specifying <<ref-cmd-config-in,`--config-in`>> and <<ref-cmd-table-file,`--table-file`>> without specifying `--[no-]overwrite-table-file` will inherit the value from <<ref-xml-tablefile,`/extTableGenConfig/externalTable/tableFile[@overwrite]`>> from the XML config.
This could lead to unintended overwrites.

We're not yet sure how we're going to resolve this inconsistency, but likely we'll remove the inheritance from the XML file if <<ref-cmd-table-file,`--table-file`>> is specified without `--[no-]overwrite-table-file`.
====
...
It does not override the column encoding of existing columns (unless you use an explicit <<ref-cmd-tbl-derive-mode>>).
...
It does not override the current end-column of `/extTableGenConfig/extTable/columns/endColumn` (unless you use an explicit <<ref-cmd-tbl-derive-mode>>).
