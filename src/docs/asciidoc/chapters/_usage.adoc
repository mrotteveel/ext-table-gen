...
// TODO Add section on using integral types
...
It can be more efficient -- both in terms of storage space and for performance loading the data into Firebird -- to use a different datatype.
...
The default setting of `byteOrder="AUTO"` is generally sufficient;
it will use the endianness of the system where _ext-table-gen_ is run.
However, if you generate the table file on a little-endian machine, while your Firebird server runs on a big-endian machine (or vice-versa), you'll need to set the `byteOrder` appropriately to match the endianness of the Firebird server.
